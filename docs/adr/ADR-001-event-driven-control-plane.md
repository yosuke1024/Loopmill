...
| `codex` | `observed` (Codex Cloud) | `subscription-login`, credential vendor-side | Dropped 2026-09-06 — SPIKE-2 NO-GO (R4, R8) |
...
| R3 | Codex: R4 (result reaches GitHub without a human click) or R7 (`ready` distinguishable from `error`, bounded in time) fails | `observed` is NO-GO; the MVP is Claude-only cross-role and "cross-vendor" is deferred → STOP condition (b) must be evaluated honestly | **SPIKE-2** (GO requires R1 ∧ R4 ∧ R5 ∧ R7 ∧ (R3 ∨ R8)) — **landed 2026-09-06**: R4 and R8 failed, `observed` dropped; cross-vendor now rests on `codex exec` (`local`, and `github-actions` pending SPIKE-2b), so STOP (b) is evaluated when SPIKE-2b lands |
