...
| 2026-09-07 | `envelope.schema.json` 1.1.0 (same version, not yet released): `$defs.usage` accepts the six audit fields of usage-normalization.md §1.2 and null for `model`, `source.runtimeVersion` and `source.eventKind`, so a completion envelope carries the stored `UsageRecord` in full; additive | amendment |
| 2026-09-07 | `loop-file.md` §8.2: a command node's `env` allowlist has no default — unset means no further restriction, an explicit empty list is honoured literally; `ResolvedCommandNode.env` becomes `string[] \| null` | clarification |
| 2026-09-07 | `usage-normalization.md` §4.1 (and `state-machine.md` §7.5): a QUOTA-classified attempt is excluded from a Node Execution's "every contributing attempt measured" test and from the coverage denominator's unmeasured set — the CLI declined before work started, it is not a partial measurement. Its own Attempt record keeps `usage.provenance: 'unavailable'` on the ledger | amendment |
| 2026-09-07 | `state-machine.md` §6.6: `progressFingerprint(cycle, nodeId) = sha256(changeFingerprint(cycle, nodeId) \|\| verdictFingerprint(cycle - 1))` — the verdict half reads the evidence available *before* the cycle being fingerprinted started, one cycle back on both sides of a NO_PROGRESS comparison, not each side's own (for the "handed" side, still-empty) same-numbered cycle | amendment |
| 2026-09-07 | `state-machine.md` §11.2 step 4: `budget.unmeasuredExecutions > budget.maxUnmeasuredExecutions`, not the pre-amendment `>=` — with `>=` and the MVP default of 0, the first dispatch of every Run would breach; `state-machine.json` `preDispatchOrder` step 4's `check` string updated to match | amendment |
| 2026-09-07 | `state-machine.md` §4.1: a note under the R-08 row states that R-01 emits `run-started` and, in the same transition, folds in the R-05..R-08 dispatch decision, so `run-requested` alone moves the Run from `∅` to `RUNNING` / `WAITING_HUMAN` / a terminal state; `loopmill step` never re-feeds an emitted envelope back in as a new inbound event (§3.3) | amendment |
| 2026-09-07 | `src/store/sqlite.ts` `rebuildSnapshot(runId, fold)`: `fold` now takes `(rows: StoredEvent[]) => RunSnapshot` (the stored rows, `kind`/`recordedAt` included), not `(events: Envelope[]) => RunSnapshot` — `engine/fold.ts`'s own `fold()` needs the recorded clock (`now = recordedAt`) to replay a transition faithfully, which a bare envelope list cannot carry. No prose in a frozen spec document named this signature; recorded here as the m1 engine/store contract it clarifies | clarification |
| 2026-09-07 | `loop-file.md` §6.3 (and `mvp-design.md` §13.2): the built-in environment preserve list gains `USER`. Measured during the first live run on the maintainer's host: without it `claude` reports `loggedIn: false` / `Not logged in · Please run /login` and never reaches the API, while `codex` authenticates without it. Additive, and `USER` is neither a credential nor vendor configuration | amendment |
